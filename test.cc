...
//     double mu=200; // unit in #
    ...
    //double p_sp, double p_mis_sp, double avgN
    double p_sp = 0.0005;
    p_sp = 0.001;
...
//     DSBUpperOrLowerBoundType = "lower"; 
...
//         mySim.SetCellWorld(0.1,0.1,0.1);//create cell world, dimension in unit of mm, this line is for testing single cell simulation
    ...
    G4String testRadiationName="Mono_Electron_Plane";
    string doseFileName;
    ...
    cout<<"DNADamageFileName is "<<DNADamageFileName<<endl;
//     exit(0);
...
//     Co_60_SpectrumPNum_2000000000_DNADamage
//     singleCellIrradiationSource.RadiationPointSourceSetUp("Co_60_SpectrumGNP_translate",10000000,particleEnergy,0,0,0.04);
    singleCellIrradiationSource.RadiationPointSourceSetUp("Mono_Electron",100000,particleEnergy,0,0,0.04);
    ...
    cout<<"DNADamageFileName_singleCell = "<<DNADamageFileName_singleCell<< endl;
    cout<< "doseFileName_singleCell = "<<doseFileName_singleCell<< endl;
//     exit(0);
...
// 	testRead.GetAbsoluteResultsByCOIMethod(8,"indirect","mean");
...
//     cellStateFileName = testSource.GetSimulationSourceID()+"_cellState.csv";
    cellSurivialFileName = testSource.GetSimulationSourceID() + "_cellSurvival.csv";
//     ofstream file;
//     ofstream file1;
    ofstream file_sf;
...
//     file1.open(cellStateFileName.c_str());
    file_sf.open(cellSurivialFileName.c_str());
...
//     for (int n =18;n<19;n++)// simulation for different prescribed dose
    for (int n =0;n<prescribedDoseVec.size();n++)// simulation for different prescribed dose
    {
        ...
        cellPhaseFileName = testSource.GetSimulationSourceID()+"_cellPhase_dose_" + cellDose +  ".csv";
        file_phase.open(cellPhaseFileName.c_str());

        ofstream file_state;
        string cellStateFileName;

        cellStateFileName = testSource.GetSimulationSourceID()+"_cellState_dose_" + cellDose +  ".csv";
...
//                    cout<<"Cell: "<<mitr_cell->first<<" State: "<<stateMap[mitr_cell->first]<<" Age: "<<ageMap[mitr_cell->first]<<" Duration: "<<durationMap[mitr_cell->first]<<endl;
            ...
            file_state<<cycle_cellStateUPdate<<","<<S1_num<<","<<S21_num<<","<<S22_num<<","<<S3_num<<","<<stateMap.size()<< endl;
        ...
        //                       cout<<"the concentration is "<<concentration<<endl;
                    concentrationInMass = concentration*massOfBystanderSignal/volume;
        //                     double concentrationInMass = 1E+4*massOfBystanderSignal/volume;
                }
// 		    cout<<"concentrationInMass "<<concentrationInMass<<endl;
                ...
                if (cycle_cellStateUPdate==1)
                {
                    ...
                    radiationTransResulut.SetSingleCellDSBUpperLowerBound(DSBUpperOrLowerBoundType);
...
//********************************************************************************************************
	    ...
	    if (mitr_cell->second =="S1" || mitr_cell->second == "S21" ) // only consider the alive cell
	    {
