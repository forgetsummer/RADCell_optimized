/**
 * @file ModelCalibrator.cc
 * @brief Implementation of model calibration class
 */

#include "ModelCalibrator.hh"
...
ModelCalibrator::ModelCalibrator(const FitConfig& config)
    : m_config(config)
    , m_model(config)
{
}

void ModelCalibrator::setConfig(const FitConfig& config) {
    m_config = config;
    m_model.setConfig(config);
}

void ModelCalibrator::setData(const std::vector<DataPoint>& data) {
    m_data = data;
}

void ModelCalibrator::addDataPoint(const DataPoint& point) {
    m_data.push_back(point);
}

void ModelCalibrator::clearData() {
    m_data.clear();
}

void ModelCalibrator::setDefaultUncertainties(double rel_err) {
...
double ModelCalibrator::negativeLogLikelihood(const ParamsReduced& params) const {
...
FitResult ModelCalibrator::fit(const ParamsReduced& initial_guess,
...
Hessian2 ModelCalibrator::computeHessian(const ParamsReduced& params,
...
std::vector<std::pair<double, double>> ModelCalibrator::profileLikelihood_e3(
...
std::vector<std::pair<double, double>> ModelCalibrator::profileLikelihood_a(
    const ParamsReduced& params_opt,
    double a_min,
    double a_max,
    int n_grid) const
...
std::vector<double> ModelCalibrator::predictSurvival(const ParamsReduced& params,
                                                      const std::vector<double>& doses) const {
    return m_model.survivalCurve(doses, params);
}

const std::vector<DataPoint>& ModelCalibrator::getData() const {
    return m_data;
}

const SurvivalModel& ModelCalibrator::getModel() const {
    return m_model;
}
