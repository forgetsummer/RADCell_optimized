...
//             cout<<"The singalling cell index is "<<c_i<<" "<<c_j<<" "<<c_k<<endl;
    ...
    double diffusion_coeff = D * deltaT / pow(d, 2);
    double stability_factor = 1 - 6 * diffusion_coeff;
    ...
    for (int n=0;n<=N;n++) // loop all the time steps
    {
        ...
        // Step 1: Diffusion calculation for ALL interior grid points (optimized)
        for (int i=1;i<gridIndex.size()-1;i++)
        {
            for (int j=1;j<gridIndex[i].size()-1;j++)
            {
                ...
                {
                    // Pure diffusion + secretion (no reaction term here)
                    CF[i][j][k] = stability_factor*CI[i][j][k]+\
                    diffusion_coeff*(CI[i+1][j][k]+CI[i-1][j][k]+CI[i][j+1][k]+CI[i][j-1][k]+CI[i][j][k+1]+CI[i][j][k-1])\
                    +secretionPerStep[i][j][k];
                } 
            }
        }
        
        // Step 2: Receptor and integral updates ONLY at cell locations (optimized)
        for (std::map<int, IJKVec>::iterator mitr_cell=cellIndex.begin();mitr_cell!=cellIndex.end();mitr_cell++)
        {
            if (cellState[mitr_cell->first] == 1 || cellState[mitr_cell->first] == 2)
            {
                int c_i = mitr_cell->second.i;
                int c_j = mitr_cell->second.j;
                int c_k = mitr_cell->second.k;
                
                // Skip boundary cells (they are handled separately)
                if (c_i >= 1 && c_i < gridIndex.size()-1 &&
                    c_j >= 1 && c_j < gridIndex[c_i].size()-1 &&
                    c_k >= 1 && c_k < gridIndex[c_i][c_j].size()-1)
                {
                    double reactionTerm = r * RI[c_i][c_j][c_k] * CI[c_i][c_j][c_k] * deltaT;
                    
                    // Subtract reaction term from concentration at cell location
                    CF[c_i][c_j][c_k] -= reactionTerm;
                    
                    // Update free receptor number
                    RF[c_i][c_j][c_k] = RI[c_i][c_j][c_k] - reactionTerm;
                    
                    // Update integral concentration
                    integralC[c_i][c_j][c_k] += reactionTerm;
                }
            }
        }
        
        // Step 3: Update RI from RF only at cell locations (optimized)
        for (std::map<int, IJKVec>::iterator mitr_cell=cellIndex.begin();mitr_cell!=cellIndex.end();mitr_cell++)
        {
            if (cellState[mitr_cell->first] == 1 || cellState[mitr_cell->first] == 2)
            {
                ...
                if (c_i >= 1 && c_i < gridIndex.size()-1 &&
                    ...
                    c_k >= 1 && c_k < gridIndex[c_i][c_j].size()-1)
                {
                    RI[c_i][c_j][c_k] = RF[c_i][c_j][c_k];
                }
            }
        }
        
        // Boundary conditions: only update at cell locations on boundaries
        for (std::map<int, IJKVec>::iterator mitr_cell=cellIndex.begin();mitr_cell!=cellIndex.end();mitr_cell++)
        {
            if (cellState[mitr_cell->first] == 1 || cellState[mitr_cell->first] == 2)
            {
                ...
                int Jm = gridIndex[c_i].size()-1;
                int Km = gridIndex[c_i][c_j].size()-1;
                
                if (c_i == 0 || c_j == 0 || c_k == 0 || c_i == Im || c_j == Jm || c_k == Km)
                {
                    double reactionTerm = r * RI[c_i][c_j][c_k] * CI[c_i][c_j][c_k] * deltaT;
                    RI[c_i][c_j][c_k] -= reactionTerm;
                    integralC[c_i][c_j][c_k] += reactionTerm;
                }
            }
        }
...
//             cout<<"The singalling cell index is: "<<c_i<<" "<<c_j<<endl;
    ...
    // Pre-compute constants for optimization
    double diffusion_coeff = D * deltaT / pow(d, 2);
    double stability_factor = 1 - 4 * diffusion_coeff;
...
//         cout<<"Get to time step: "<<n<<endl;
        ...
        for (int i=1;i<gridIndex2D.size()-1;i++)
        {
            ...
            {          
                // Pure diffusion + secretion (no reaction term here)
                CF[i][j] = stability_factor*CI[i][j]+diffusion_coeff*(CI[i+1][j]+CI[i-1][j]+CI[i][j+1]+CI[i][j-1])\
                +secretionPerStep[i][j];
            }
        }
        ...
        for (std::map<int, IJVec>::iterator mitr_cell=cellIndex2D.begin();mitr_cell!=cellIndex2D.end();mitr_cell++)
        {
            if (cellState[mitr_cell->first] == 1 || cellState[mitr_cell->first] == 2)
            {
                ...
                // Skip boundary cells (they are handled separately)
                if (c_i >= 1 && c_i < gridIndex2D.size()-1 &&
                    c_j >= 1 && c_j < gridIndex2D[c_i].size()-1)
                {
                    double reactionTerm = r * RI[c_i][c_j] * CI[c_i][c_j] * deltaT;
                    
                    // Subtract reaction term from concentration at cell location
                    CF[c_i][c_j] -= reactionTerm;
                    
                    // Update free receptor number
                    RF[c_i][c_j] = RI[c_i][c_j] - reactionTerm;
                    
                    // Update integral concentration
                    integralC[c_i][c_j] += reactionTerm;
                }
            }
        }
        ...
        for (std::map<int, IJVec>::iterator mitr_cell=cellIndex2D.begin();mitr_cell!=cellIndex2D.end();mitr_cell++)
        {
            if (cellState[mitr_cell->first] == 1 || cellState[mitr_cell->first] == 2)
            {
                ...
                int Jm = gridIndex2D[c_i].size()-1;
                
                // Check if cell is on any boundary
                if (c_i == 0 || c_j == 0 || c_i == Im || c_j == Jm)
                {
                    double reactionTerm = r * RI[c_i][c_j] * CI[c_i][c_j] * deltaT;
                    RI[c_i][c_j] -= reactionTerm;
                    integralC[c_i][c_j] += reactionTerm;
                }
            }
        }
